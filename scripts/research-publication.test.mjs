...
function loadTS(relative) {
  ...
  const mod = { exports: {} };
  new Function("require", "exports", "module", outputText)(name => name.startsWith("@/") ? loadTS(`src/${name.slice(2)}.ts`) : require(name), mod.exports, mod);
  return mod.exports;
}
...
test("one published article retains evidence limitations and no PDF or product claims", () => {
  assert.equal(RESEARCH_PUBLICATIONS.length, 1);
  const p = RESEARCH_PUBLICATIONS[0];
  assert.equal(p.slug, "asi-research-001");
...
test("native article renders all 11 sections, links and qualified core claim", () => {
  ...
  assert(html.includes(RESEARCH_PUBLICATIONS[0].coreClaim));
...
test("link resolver rejects unsafe schemes and arbitrary relative paths", () => {
  ...
  assert.equal(researchLinkHref("METHODOLOGY.md"), RESEARCH_PUBLICATIONS[0].methodologyUrl);
});
